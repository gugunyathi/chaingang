...
function Index() {
  ...
  const PlayerStrip = () => (
    <section
      ...
      className="scrollbar-hide flex gap-4 overflow-x-auto border-b border-border bg-background px-4 py-4 sm:rounded-lg sm:border sm:bg-card"
    >
      {players.map((player, index) => (
        <Button
          ...
          className="group flex h-auto w-14 shrink-0 flex-col items-center gap-1.5 p-0"
          aria-pressed={selectedPlayer === player}
        >
          <span
            className={`grid size-12 place-items-center rounded-full border-2 text-xs font-bold transition-transform group-active:scale-95 ${selectedPlayer === player ? "border-primary bg-primary text-primary-foreground" : index < 3 ? "border-live bg-secondary" : "border-border bg-secondary"}`}
          >
  ...
  const LivePanel = ({ mobile = false }: { mobile?: boolean }) => (
    ...
    >
      ...
      <div
        className={`relative flex flex-col p-4 sm:p-6 ${mobile ? "min-h-[calc(100dvh-9.5rem)]" : "min-h-[680px]"}`}
      >
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
          <div className="min-w-0">
            <p className="text-xs font-bold uppercase text-primary">On camera now</p>
            <h1 className="truncate font-display text-2xl font-bold">{selectedPlayer}</h1>
          </div>
          <Button
        ...
        <div className="flex flex-1 flex-col items-center justify-center py-6 text-center">
          ...
          <p className="mt-1 font-display text-5xl font-bold tabular-nums sm:text-7xl">
            $1,284<span className="text-primary">.57</span>
          </p>
          <Button
            variant="ghost"
            onClick={toggleLike}
            ...
            aria-pressed={liked}
            className="relative mt-8 size-44 rounded-full border-4 border-primary/30 bg-background/30 p-0 shadow-glow transition-transform active:scale-95 sm:size-56"
          >
          ...
          <div className="mt-6 flex gap-2">
            {[5, 4, 3, 2, 1].map((n) => (
              <span
                key={n}
                className={`grid size-9 place-items-center rounded-md border text-sm font-bold ${n === count ? "border-primary bg-primary text-primary-foreground shadow-glow" : n > count ? "border-border bg-secondary text-muted-foreground" : "border-border/50 text-muted-foreground/40"}`}
              >
                {n}
              </span>
            ))}
          ...
          <div className="grid grid-cols-[minmax(0,1fr)_auto] gap-3">
            <Button
              ...
              onClick={() => {
                ...
                notify(queued ? "You left the queue" : "You joined with 10 tickets");
              }}
            >
              {queued ? (
                <>
  ...
  const ActivityPanel = ({ mobile = false }: { mobile?: boolean }) => (
    <Panel id={mobile ? undefined : "feed"} className={mobile ? "min-h-full border-b-0 pr-16" : ""}>
      ...
      <div className="mt-5 space-y-3">
        {activity.map((item) => (
          <Button
            ...
            className="grid h-auto w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 rounded-lg border border-transparent bg-secondary p-3 text-left hover:border-primary/40"
          >
            <span
              className={`grid size-10 place-items-center rounded-md text-[10px] font-bold ${item.kind === "WIN" ? "bg-accent text-accent-foreground" : "bg-primary text-primary-foreground"}`}
            >
              {item.kind}
            </span>
            <span className="min-w-0">
              ...
              <span className="mt-1 block text-[10px] text-muted-foreground">{item.time} ago</span>
            </span>
  ...
  const QueuePanel = ({ mobile = false }: { mobile?: boolean }) => (
    <Panel
      id={mobile ? undefined : "queue"}
      className={mobile ? "min-h-full border-b-0 pr-16" : ""}
    >
      <div className="flex items-center justify-between">
        <div>
  ...
  const SettingsPanel = ({ mobile = false }: { mobile?: boolean }) => (
    <Panel
      id={mobile ? "mobile-settings" : "wallet"}
      className={mobile ? "min-h-full border-b-0 pr-16" : ""}
    >
      <div>
      ...
      <div className="mt-5 space-y-2">
        {[
          ...
          { icon: Volume2, label: "Sound effects", active: sound, set: () => setSound((v) => !v) },
        ].map((item) => (
          <div
            key={item.label}
      ...
      <div className="mt-6" id={mobile ? "mobile-wallet" : undefined}>
        <p className="text-xs font-bold uppercase text-primary">Wallet</p>
        <Button
          variant="vaultOutline"
          size="touch"
          className="mt-2 w-full justify-between"
          onClick={() => notify("Wallet connection is ready for setup")}
        >
      ...
      <div className="mt-2 grid gap-2">
        {["Cash out", "PvP bet · from $0.20", "Keep playing AI bets"].map((option) => (
          <Button
            ...
            className={`grid h-auto grid-cols-[minmax(0,1fr)_auto] items-center gap-2 rounded-lg border p-3 text-left text-sm font-semibold ${winnerOption === option ? "border-primary bg-primary/10 text-primary" : "border-border"}`}
          >
            <span>{option}</span>
            {winnerOption === option ? (
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="sm:hidden">
        ...
        >
          ...
          >
            <article className="h-[calc(100dvh-9.25rem)] w-full shrink-0 overflow-y-auto overscroll-contain pr-12">
              <LivePanel mobile />
            </article>
            <article className="h-[calc(100dvh-9.25rem)] w-full shrink-0 overflow-y-auto overscroll-contain">
        ...
        <div
          className="fixed bottom-3 left-1/2 z-40 flex -translate-x-1/2 items-center gap-2 rounded-full border border-border bg-background/90 px-3 py-2 backdrop-blur-xl"
        ...
        >
          {pageNames.map((name, index) => (
            <Button
              ...
              className={`size-2 rounded-full p-0 ${index === mobilePage ? "bg-primary" : "bg-muted-foreground/40"}`}
            />
          ))}
        </div>
      </main>
      ...
      <nav
        ...
        className="fixed right-2 top-1/2 z-40 flex -translate-y-1/2 flex-col gap-2 rounded-lg border border-border bg-background/90 p-1.5 shadow-xl backdrop-blur-xl sm:hidden"
      >
        {[
          { icon: Radio, label: "Timeline", page: 0 },
          { icon: Trophy, label: "Activity", page: 1 },
          { icon: ListOrdered, label: "Hash order", page: 2 },
          { icon: Wallet, label: "Wallet", page: 3 },
          { icon: Settings, label: "Settings", page: 3 },
        ].map((item) => (
          <Button
            key={item.label}
            variant="ghost"
            ...
            aria-current={mobilePage === item.page ? "page" : undefined}
            onClick={() => selectPage(item.page)}
            className={`size-10 rounded-md ${mobilePage === item.page ? "bg-primary text-primary-foreground shadow-vault" : "text-muted-foreground"}`}
          >
      ...
      <main className="mx-auto hidden max-w-6xl px-4 pb-8 pt-4 sm:block">
        <PlayerStrip />
        <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)] lg:items-start">
          ...
          <aside className="min-w-0 space-y-4 lg:sticky lg:top-20">
            ...
            <Panel id="target">
              <div className="flex items-center gap-2 text-primary">
                ...
                <p className="text-xs font-bold uppercase">AI target · #419</p>
              </div>
              <h2 className="mt-1 font-display text-xl font-bold">Build the missing face</h2>
              <div className="mt-3 flex flex-wrap gap-2">
                {["half-smile", "raised brow", "side light", "green jacket", "wide eyes"].map(
                  (tag) => (
                    <Button
                      variant="vaultOutline"
                      size="sm"
                      key={tag}
                      onClick={() => notify(`${tag} highlighted`)}
                    >
                      {tag}
                    </Button>
                  ),
      ...
      {mobilePage > 0 && (
        <Button
          variant="vaultOutline"
          size="icon"
          className="fixed bottom-3 left-3 z-40 sm:hidden"
